...
class BackpackPerpetualWebUtilsUnitTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.pre_processor = BackpackPerpetualRESTPreProcessor()
    ...
    @aioresponses()
    def test_backpack_perpetual_get_server_time(self, req_mock):


        url = web_utils.public_rest_url(
            CONSTANTS.SERVER_TIME_PATH_URL, domain=CONSTANTS.DEFAULT_DOMAIN
        )
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))

        req_mock.get(regex_url, body="1743776827119")

        result = self.async_run_with_timeout(
            web_utils.get_current_server_time(throttler=None, domain=CONSTANTS.DEFAULT_DOMAIN)
        )

        self.assertIsInstance(result, float)
        self.assertEqual(result, 1743776827119)


    def test_backpack_perpetual_rest_pre_processor_non_post_request(self):
    ...
    def test_backpack_perpetual_rest_pre_processor_post_request(self):
        request: RESTRequest = RESTRequest(
